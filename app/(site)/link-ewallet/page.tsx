...
export default function LinkEWalletPage() {
  useLightSheet();
  const { toast } = useUI();
  ...
  const add = async (e: React.FormEvent) => {
    ...
    /* the fund password gates adding a payout account, the same way it gates
       a withdrawal — but only once the player has one to check against */
    if (hasTxnPassword) {
    ...
    setGroup(null); setNo(''); setPass(''); setErr('');
    await load();
          ...
          <form className="ms-pad ms-lwform" onSubmit={add} noValidate>
            ...
            {hasTxnPassword && (
              <label className="ms-field">
                ...
                <input
                  type={showPass ? 'text' : 'password'} placeholder="＊ Transaction Password"
                  autoComplete="off" value={pass}
            ...
            <button
              ...
              disabled={busy || clean.length < 4 || (hasTxnPassword && !pass)}
            >
              {busy ? '…' : 'Submit'}
            </button>
          </form>
